...
export default function QA() {
    ...
    return (
        <div>
            <h1 style={{textAlign:'center'}}> 常見問題</h1>
            <h6 style={{textAlign:'center'}}>我們匯集了許多常見問題的解答，若您的問題仍未解決，請透過電話或電子信箱聯繫我們。</h6>
            <Collapse onChange={onChange} style={{ borderRadius: '0px',margin:'1rem auto' }} bordered={false}  className={styles.cc}
            >
                <Panel header="關於訂單" key="1"  className={styles.header}>
                    <Collapse style={{ borderRadius: '0px' }} bordered={false}>
                        <Panel header="如何下單？" key="1"  className={styles.header}>
                        ...
                        <Panel header="如何查詢訂單狀態？" key="2"  className={styles.header}>
                            <p className={styles.p}>A : 完成下單動作後，點選會員圖示進入會員帳戶頁面，尋找欲查詢訂單狀態的商品並點選該商品，點選商品下排的狀態查詢，便能看到目前該商品。
                            </p>
                        </Panel>
                        <Panel header="訂單如何取消或更改？" key="3" className={styles.header}>
                ...
                <Panel header="關於付款" key="2"  className={styles.header}>
                    <Collapse style={{ borderRadius: '0px' }} bordered={false}>
                        <Panel header="接受哪些付款方式？" key="1" className={styles.header}>
                            <p className={styles.p}>預先付款則接受信用卡，金融卡或銀行轉帳的方式付款。
                            </p>
                        </Panel>
                    </Collapse>
                </Panel>
                <Panel header="關於商品" key="3"  className={styles.header}>
                    <Collapse style={{ borderRadius: '0px' }} bordered={false}>
                        <Panel header="商品有無保固？" key="1"  className={styles.header}>
                            <p className={styles.p}>A : 公仔項目商品有提供三個月的保固期。</p>
                        </Panel>
                        <Panel header="商品的鑑賞期限？" key="2"  className={styles.header}>
                            <p className={styles.p}>A : 有，各項商品在買家收到商品後有十天的鑑賞期，如欲退換貨請在這十天內辦理手續。</p>
                        </Panel>
                        <Panel header="如何選擇適合自己的商品？" key="3"  className={styles.header}>
                            <p className={styles.p}>A : 先想好欲購買何種主題的書籍或哪部作品周邊，選擇自己欲買的商品種類，可考慮自家放空間，或個人風格等因素挑選欲購買的作品相關周邊商品大小和樣式。
                            </p>
                        </Panel>
                    </Collapse>
                </Panel>
                <Panel header="關於配送" key="4"  className={styles.header}>
                    <Collapse style={{ borderRadius: '0px' }} bordered={false}>
                        <Panel header="有哪些配送方式？" key="1" className={styles.header}>
                            <p className={styles.p}>A:國際快遞、一般宅配。</p>
                        </Panel>
                        <Panel header="配送需要多久？" key="2"  className={styles.header}>
                            <p className={styles.p}>A : 依該商品的配送距離及下單時期而有所差異。<br />
                                海內的配送，一般在接受訂單後在七到十四日內會完成配送，若是在特殊時期，則有可能需十四日到一個月的時間。<br />
                                海外的配送，則可能需要三個月至一年的時間。
                            </p>
                        </Panel>
                        <Panel header="可否提供國際配送？" key="3"  className={styles.header}>
                            <p className={styles.p}>A : 會，但國際配送耗時較久，如商品有需要退貨或是更改的狀況也會需要較多時間處裡。
                            </p>
                        </Panel>
                    </Collapse>
                </Panel>
                <Panel header="關於售後服務" key="5"  className={styles.header}>
                    <Collapse style={{ borderRadius: '0px' }} bordered={false}>
                        <Panel header="如何聯繫客服？" key="1"  className={styles.header}>
                        ...
                        <Panel header="如何辦理退換貨？" key="2"  className={styles.header}>
                        ...
                        <Panel header="售後服務有哪些保障？" key="3"  className={styles.header}>
